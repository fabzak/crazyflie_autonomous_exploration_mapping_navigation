...
def generate_launch_description():
    ...
    # Models directory inside the installed gazebo package
    gz_models_path = os.path.join(pkg_project_gazebo, 'models')

    # Append to any existing resource path so other models still work
    existing = os.getenv('GZ_SIM_RESOURCE_PATH', '')
    new_resource_path = gz_models_path if not existing else f"{existing}:{gz_models_path}"
